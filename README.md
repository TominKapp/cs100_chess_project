...
The two design patterns we decided on are first:
     ...
     * The second design pattern we will be using is the strategy design pattern. This will allow us to assign unique movement algorithms to each piece without extending the piece class for each instance. For example, each unique chess piece has a `possible_moves` function which originates from a general piece class, this function will call the unique movement strategy for each type of piece based on the value assigned to the piece instance. For example, a bishop's `possible_moves` function would in turn call the `move_bishop` strategy.
